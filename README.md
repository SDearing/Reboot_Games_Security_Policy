...
A disaster recovery (DR) plan are the policies and procedures put into place by the organisation when IT services are disrupted. This could be caused by events like a fire, technological failure or human factors such as sabotage. The main purpose of the DR plan is to restore the companies procedures as quickly as possible either by switching to contingency systems or by restoring the disrupted systems.

### The Components of a Disaster Recovery Plan with Justifications
#### Introduction
This is where the the objectives of the plan are summarized, aswell as the scope of the plan, meaning the IT services involved and the location(s) of the organisation covered in the plan. The objectives are the recovery point objective (RPO) and the recovery time objective (RTO). RPO describes the age of a back up when it stops being useful, for example if a organisation cannot afford to lose a days worth of data, the RPO would be set to 24 hours. The RTO describes the maximum amount of time that should be allowed to occur before the backup is implemented and normal services are resumed. The introduction is a useful component to a DR plan because it gives a quick run down of the plan, and can be useful to help someone, who is not familiar with the plan, to understand it more effeciently.
#### Roles and Responsibilities
This is where the responsibilities of all employees are made clear. Roles are given to every member of the organisation along with each members contact details so they can be contacted. It is important to spread out tasks of the DR plan to different members of the organisation so that there is no confusion of what each members role is in a disaster and so issues can be completed more effeciently. 
#### Incident Response
This is where the situations of when the DR plan should be triggered are made clear and when and how all internal and external stakeholders should be notified. This component is important because it clearly outlines when the DR plan should be implemented, avoiding the issue of the DR plan being implemented at the wrong time.
#### Disaster Recovery Procedures
This is where the DR process can be created for every possible affected IT service. This is needed because every IT service is going to need a different procedure depending on the situation, so it is important the the creator of the DR plan accounts for every possibility and gives a step-by-step procedure for every situation. This component is the most important component of the DR plan as it needs to be clear enough that the employees following the procedures can solve the issues with complete effectiveness, because if they do not then the data will not be recovered effectivley costing the organisation a huge amount.
